module Tests.XLIFF
    ...
    [<Theory>]
    [<InlineData(0)>]
    let ``XLIFF_v1_2_Class ReadDontValidateXliff ERROR has foo element`` (index:int) =
        let instance = XLIFF_v1_2_Class()
        let action = fun () -> instance.ReadDontValidate(xliff_broken_samples.[index]);
    ...
    [<InlineData(7)>]
    [<InlineData(8)>]
    let ``XLIFF_v1_2_Class Write with data`` (index:int) =
        ...
        let instance = XLIFF_v1_2_Class()
        let readAction = fun () -> instance.ReadDontValidate(xliff_samples.[index])
        readAction()
    ...
    [<Theory>]
    [<InlineData(0)>]
    let ``XLIFF_v1_2_Class WriteXliffFile with no data throws an exception`` (index:int) =
        let instance = XLIFF_v1_2_Class()
        let action = fun () -> instance.Write("file")
        try
            let result = action()
            Assert.True(false, "Expected an exception when running XLIFF_v1_2_Class WriteXliffFile with no data")
        with 
        | ex -> let action_match = ex.Message.Contains("Not read!")
                Assert.True(action_match, "XLIFF_Class with no data is supposed to throw a (" + ex.Message + ") exception.")
    ...
    [<InlineData(7)>]
    [<InlineData(8)>]
    let ``XLIFF_v1_2_Class ReadResxOrResw`` (index:int) =
        let instance = XLIFF_v1_2_Class()
        let action = fun () -> instance.ReadResxOrResw("file"); true
        try
            let result = action()
            Assert.True(result)
        with 
    ...
    let ``XLIFF_v1_2_Class WriteResxOrReswFile with no data`` (index:int) =
        let instance = XLIFF_v1_2_Class()
        let action = fun () -> instance.WriteResxOrResw("file")
        try
            let result = action()
            Assert.True(result)
        with 
    ...
    let ``XLIFF_v1_2_Class CheckResxOrReswFile with no data`` (index:int) =
        let instance = XLIFF_v1_2_Class()
        let action = fun () -> instance.CheckResxOrReswForXliff("file")
        try
            let result = action()
            Assert.True(result)
        with 
