module Tests.RESX_RESW
    ...
    [<InlineData(0)>]
    [<InlineData(1)>]
    let ``RESX_Class ReadResxFile has ERROR`` (index:int) =
        let instance = RESX_Class()
        let action = fun () -> instance.ReadResxFile(resx_error_samples.[index]);
    ...
    [<InlineData(6)>]
    [<InlineData(7)>]
    let ``RESX_Class ReadResxFile`` (index:int) =
        let instance = RESX_Class()
    ...
    [<InlineData(6)>]
    [<InlineData(7)>]
    let ``RESX_Class WriteResxFile with data`` (index:int) =
        ...
        let instance = RESX_Class()
        let readAction = fun () -> instance.ReadResxFile(resx_samples.[index])
        readAction()
        ...
        let writeAction = fun () -> instance.WriteResxFile(writeFilePath);
        try
            writeAction()
    ...
    [<Theory>]
    [<InlineData(0)>]
    let ``RESX_Class WriteXliffFile with no data`` (index:int) =
        let instance = RESX_Class()
        let action = fun () -> instance.WriteResxFile(resx_samples.[index])
        try
            let result = action()
    ...
    [<InlineData(7)>]
    [<InlineData(8)>]
    let ``RESX_Class WriteResxOrReswFile with no data`` (index:int) =
        let instance = RESX_Class()
        let action = fun () -> instance.WriteResxFile(resx_samples.[index]); false
        try
            let result = action()
            Assert.True(result)
        with 
    ...
    [<InlineData(7)>]
    [<InlineData(8)>]
    let ``RESX_Class CheckResxFile with no data`` (index:int) =
        let instance = RESX_Class()
        let action = fun () -> instance.CheckResxFile(resx_samples.[index])
        try
            match action() with 
            | None -> Assert.True(true)
            | Some _ -> Assert.True(false)
    ...
    [<InlineData(6)>]
    [<InlineData(7)>]
    let ``RESX_Class CheckResxFile with data`` (index:int) =
        let instance = RESX_Class()
        let action = fun () -> instance.CheckResxFile(resx_samples.[index])
        try
            match action() with 
