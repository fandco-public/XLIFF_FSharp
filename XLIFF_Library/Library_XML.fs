...
module Library_XML = 
    ...
    type XML_Class( pSchemaPaths: string list option) = 
        ...
        // Function saying if XDocument is loaded
        member this.IsDocumentEmpty() : bool = this.Document.Root = null
        ...
        member this.ReadDontValidate(xmlPath : string) : unit =
            ...
            // Parse the RESX file
            this.Document <- XDocument.Load(xmlReader)
            xmlReader.Close()
            printfn "XML document is loaded."


        // Define a function to read & validate the RESX file
        member this.ReadAndValidate (xmlPath: string) : int option =                
        ...
        member this.Write(filePath) =
            ...
            // test if this.Document is empty
            if this.IsDocumentEmpty() then
                raise (new System.Exception("No data to write!")) |> ignore                
            
            if  this.Problems <> None then
